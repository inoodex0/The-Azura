    ...
    children: [
      {
        ...
        href: "/rooms/executive-room-double",
      },
      {
        name: "Executive Room (Twin)",
        href: "/rooms/executive-room-twin",
      },
    ],
  },
  {
        ...
        <div className="mx-auto max-w-[1500px] px-4 sm:px-6 lg:px-10">
          <div className="flex h-[70px] items-center justify-between sm:h-[80px] lg:h-[86px]">
            ...
            <div className="hidden h-full items-center lg:flex">
              <Link href="/" className="group relative flex h-full items-center px-4 text-[11px] font-semibold uppercase tracking-[0.08em] text-black transition-colors duration-300 hover:text-[#ff784e]">
                Home
               ...
               <Link href="/" className="group relative flex h-full items-center px-4 text-[11px] font-semibold uppercase tracking-[0.08em] text-black transition-colors duration-300 hover:text-[#ff784e]">
                 Rooms & Suites
                 <ChevronDown size={13} strokeWidth={1.8} className="transition-transform duration-300 group-hover:rotate-180" />
                <div className="pointer-events-none invisible absolute left-0 top-full w-[240px] translate-y-3 bg-black opacity-0 shadow-2xl transition-all duration-300 group-hover:pointer-events-auto group-hover:visible group-hover:translate-y-0 group-hover:opacity-100">
                  <div className="h-[3px] bg-[#ff784e]" />
                  {rooms.map((room) => (
                    <div key={room.name} className="group/room relative">
                      {room.children ? (
                        <>
                          ...
                          <div className="pointer-events-none invisible absolute left-full top-0 w-[230px] bg-black opacity-0 transition-all duration-300 group-hover/room:pointer-events-auto group-hover/room:visible group-hover/room:opacity-100">
                            <div className="h-[3px] bg-[#ff784e]" />
              ...
              {/* <div className="group relative h-full">
                <button type="button" className="relative flex h-full items-center gap-1.5 px-4 text-[11px] font-semibold uppercase tracking-[0.08em] text-black transition-colors duration-300 group-hover:text-[#ff784e]">
                  Rooms & Suites
                  <ChevronDown size={13} strokeWidth={1.8} className="transition-transform duration-300 group-hover:rotate-180" />
                  <span className="absolute bottom-0 left-1/2 h-[2px] w-0 -translate-x-1/2 bg-[#ff784e] transition-all duration-300 group-hover:w-5" />
                </button>
              ...
              {navItems.filter((item) => item.name !== "Home").map((item) => (
                <Link key={item.name} href={item.href} className="group relative flex h-full items-center px-4 text-[11px] font-semibold uppercase tracking-[0.08em] text-black transition-colors duration-300 hover:text-[#ff784e]">
                  {item.name}
                  <span className="absolute bottom-0 left-1/2 h-[2px] w-0 -translate-x-1/2 bg-[#ff784e] transition-all duration-300 group-hover:w-5" />
                </Link>
              ))}
            ...
            <div className="hidden items-center gap-3 sm:gap-4 lg:flex">
              <button type="button" onClick={() => setSearchOpen(true)} className="flex h-9 w-9 items-center justify-center rounded-full border border-black/10 text-black transition-all duration-300 hover:border-[#ff784e] hover:text-[#ff784e] lg:h-10 lg:w-10" aria-label="Search">
                <Search size={16} strokeWidth={1.8} className="lg:size-[17px]" />
              </button>

              <div className="hidden h-7 w-px bg-black/10 xl:block" />

              <Link href="/my-booking" className="hidden text-[11px] font-semibold uppercase tracking-[0.07em] text-black transition-colors duration-300 hover:text-[#ff784e] xl:block">
                My Booking
              </Link>

              <div className="hidden h-7 w-px bg-black/10 xl:block" />

              <Link href="/booking" className="group flex items-center gap-3 bg-black py-2 pl-4 pr-1.5 text-[10px] font-semibold uppercase tracking-[0.08em] text-white transition-all duration-300 hover:bg-[#ff784e] sm:pl-5 sm:pr-2 sm:text-[11px]">
                <span>Book Now</span>
                <span className="flex h-7 w-7 items-center justify-center bg-[#ff784e] text-white transition-all duration-300 group-hover:bg-white group-hover:text-[#ff784e] sm:h-8 sm:w-8">
        ...
        >
          <div className="mx-auto max-w-[1500px] px-3 py-3 sm:px-6 sm:py-5">
            ...
            {/* <div className="border-b border-black/[0.07]">
              ...
              <div className={`overflow-hidden transition-all duration-300 ${mobileRoomsOpen ? "max-h-[700px] pb-2" : "max-h-0"}`}>
                {rooms.map((room) => (
                  <div key={room.name}>
                    {room.children ? (
                      <>
                        <button type="button" onClick={() => setMobileSubmenu(mobileSubmenu === room.name ? null : room.name)} className="flex w-full items-center justify-between rounded-lg px-4 py-2.5 text-left text-[13px] text-black transition-colors hover:bg-black/[0.04]">
                          <span>{room.name}</span>
                          <ChevronRight size={15} className={`transition-transform duration-300 ${mobileSubmenu === room.name ? "rotate-90 text-[#ff784e]" : "text-black/40"}`} />
                        </button>
                        <div className={`ml-4 overflow-hidden border-l-2 border-[#ff784e] transition-all duration-300 ${mobileSubmenu === room.name ? "max-h-40 opacity-100" : "max-h-0 opacity-0"}`}>
                          {room.children.map((child) => (
                            <Link key={child.name} href={child.href} onClick={closeMobileMenu} className="block px-4 py-2 text-[12px] text-black/70 transition-colors hover:text-[#ff784e]">
                              {child.name}
                            </Link>
                          ))}
                        </div>
                      ...
                      <Link href={room.href} onClick={closeMobileMenu} className="block rounded-lg px-4 py-2.5 text-[13px] text-black transition-colors hover:bg-black/[0.04] hover:text-[#ff784e]">
                        {room.name}
                      </Link>
                    )}
                  </div>
            ...
            {["About", "Facilities", "Contact"].map((name) => (
            ...
            <Link href="/my-booking" onClick={closeMobileMenu} className="mt-3 block rounded-xl border border-black/10 px-4 py-2.5 text-center text-[12px] font-semibold uppercase tracking-wide text-black transition-all hover:border-[#ff784e] hover:text-[#ff784e] sm:mt-4 sm:py-3.5 sm:text-[13px]">
              My Booking
            </Link>
